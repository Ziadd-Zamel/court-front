...
export default function ReusableTabs({
  ...
  return (
    <Tabs
      dir={direction}
      className={`w-full ${className} mt-20`}
      defaultValue={defaultTab}
    >
      <div className="flex flex-col lg:flex-row md:items-start gap-20 w-full">
        {/* Tab list */}
        <TabsList
          className={`flex flex-col items-center gap-1 md:max-w-[300px] w-full bg-transparent mt-40 ${tabListClassName}`}
        >
          {showSearch && (
        ...
        <div className="w-full">
          {tabs.map((tab) => (
            <TabsContent
              key={tab.value}
              className={`mt-20 lg:mt-16 w-full ${tabContentClassName}`}
              value={tab.value}
            >
              {tab.heading && (
